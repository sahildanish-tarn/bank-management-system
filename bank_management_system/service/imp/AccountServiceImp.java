...
import com.jsp.bank_management_system.exception.AccountNotFoundException;
...
public class AccountServiceImp implements AccountService {
    ...
    public  ResponseEntity<AccountResponseDto> findByAccountNumber(String accountNumber) {
        Account account=accountRepository.findByAccountNumber(accountNumber).
                orElseThrow(()-> new AccountNumberDoesNotExists("Account Not Found with accountNumber: "+accountNumber));
    ...
    public ResponseEntity<AccountResponseDto> updateStatus(String accountNumber, AccountStatus status) {
        Account account =accountRepository.findByAccountNumber(accountNumber).orElseThrow(
                ()-> new AccountNumberDoesNotExists("Account Not Found With accountNumber: "+accountNumber)
        );
        if(account.getStatus()==AccountStatus.CLOSED){
            throw new AccountAlreadyClosedException("Cannot update account with closed status");
        }
    ...
    public ResponseEntity<AccountResponseDto> closeAccount(String accountNumber) {
        ...
        if (account.getBalance().compareTo(BigDecimal.ZERO)!=0) {
            throw new IllegalArgumentException("Account cannot be closed because balance is not zero");
        }
